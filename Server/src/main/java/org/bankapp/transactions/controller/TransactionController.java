...
public class TransactionController {
    ...
    public static void transfer(Context ctx) {
        Transaction transaction = ctx.bodyAsClass(Transaction.class);
        int accountId = Integer.parseInt((ctx.pathParam("accountTransferToId")));
        try {
            Account account = transactionService.createTransaction(transaction, accountId);
            ctx.status(201).json(account);
            logger.info("New transaction created: " + transaction.toString());
        } catch (SQLException e) {
            ctx.status(500).json("Internal server error");
    ...
    public static void getAllByAccount(Context ctx) {
        try {
